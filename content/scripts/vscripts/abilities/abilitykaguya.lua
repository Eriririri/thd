...
function OnKaguya01SpellThink(keys)
	...
	for _,v in pairs(targets) do
		local damage_table = {
			    ...
			    victim = v,
			    attacker = caster,
			    damage = keys.ability:GetAbilityDamage() + FindTalentValue(caster,"special_bonus_unique_lina_2")+(caster:GetIntellect()*1.4),
			    damage_type = keys.ability:GetAbilityDamageType(), 
	    	    damage_flags = 0
		}
		UnitDamageTarget(damage_table)
	end
	local targetcreep = FindUnitsInRadius(
				   caster:GetTeam(),						--caster team
				   damageVector,							--find position
				   nil,										--find entity
				   keys.DamageRadius,						--find radius
	...
	for _,v in pairs(targetcreep) do
		local damage_table = {
				ability = keys.ability,
			    victim = v,
			    attacker = caster,
			    damage = (keys.ability:GetAbilityDamage() + FindTalentValue(caster,"special_bonus_unique_lina_2")+(caster:GetIntellect()*1.4))*2,
			    damage_type = keys.ability:GetAbilityDamageType(), 
	    	    damage_flags = 0
		}
		UnitDamageTarget(damage_table)
