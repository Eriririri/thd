...
function MargatroidEx_IntervalAddDoll(keys)
	...
	local MaxStoreNum=keys.DollBaseNum+math.floor((Caster:GetLevel()-1)/keys.LvlToUpgrade)+FindTalentValue(castertalent,"special_bonus_unique_warlock_2")+FindTalentValue(castertalent,"special_bonus_unique_mirana_4")
	...
	if Margatroid_GetTotalDollNum(Caster)<MaxStoreNum and not Caster:HasModifier(MARGATROID_ABILITYEX_MODIFIER_TIME_TO_ADD_DOLL_NAME) then
		AbilityEx:ApplyDataDrivenModifier(Caster,Caster,MARGATROID_ABILITYEX_MODIFIER_TIME_TO_ADD_DOLL_NAME,{Duration=5-FindTalentValue(castertalent,"special_bonus_unique_tiny_3")})
	end
...
function Margatroid01_OnSpellStart(keys)
	...
	THDReduceCooldown(keys.ability,-FindTalentValue(castertalent,"special_bonus_unique_lich_3"))
...
function Margatroid01_IfKnockback(keys)
	...
	if not Target:HasModifier(keys.ModifierKnockbackName) then
		...
		UnitDamageTarget{
			...
			victim = Target,
			attacker = Caster,
			damage = Ability:GetAbilityDamage() + FindTalentValue(castertalent,"special_bonus_unique_leshrac_1") + (keys.caster:GetIntellect()*1.6) ,
						...
						for __,enemy in pairs(enemies) do
							...
							if math.abs(after_rotate_pos.y-doll_pos.y)<keys.LaserRadius then
								UnitDamageTarget{
									...
									victim = enemy,
									attacker = Caster,
									damage = Ability:GetAbilityDamage() + FindTalentValue(castertalent,"special_bonus_unique_antimage_2") + (keys.caster:GetIntellect()*1),
									damage_type = Ability:GetAbilityDamageType()
								}
							end
						end
...
function Margatroid03_OnSpellStart(keys)
	...
	doll:SetBaseMoveSpeed(keys.MoveSpeed+FindTalentValue(castertalent,"special_bonus_unique_mirana_3"))
...
function Margatroid32_OnSpellStart(keys)
	...
	UnitDamageTarget{
			...
			damage = Ability:GetAbilityDamage()+(keys.caster.ability_alice_03_owner:GetIntellect()*0.5),
			damage_type = Ability:GetAbilityDamageType()
		}    
end
...
function Margatroid33_OnSpellStart(keys)
	...
	local damage_table={
			...
			victim = nil,
			attacker = keys.caster.ability_alice_03_owner,
			damage = Ability:GetAbilityDamage()+(keys.caster.ability_alice_03_owner:GetIntellect()*1),
			damage_type = Ability:GetAbilityDamageType()
		}  
...
function Margatroid04_DamageTargets(keys)
	...
	for k,v in pairs(targets) do
		UnitDamageTarget{
			...
			victim = v,
			attacker = Caster,
			damage = keys.explosion_damage + FindTalentValue(Caster,"special_bonus_unique_visage_2"),
			damage_type = Ability:GetAbilityDamageType()
		}
	end
end
...
function Margatroid34_DamageTargets(keys)
	...
	for k,v in pairs(targets) do
		UnitDamageTarget{
			...
			victim = v,
			attacker = Caster,
			damage = keys.explosion_damage + FindTalentValue(Caster,"special_bonus_unique_visage_2") +(keys.caster.ability_alice_03_owner:GetIntellect()*3),
			damage_type = Ability:GetAbilityDamageType()
		}
	end
end