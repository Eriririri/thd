...
if is_spell_blocked(keys.target) then return end	
	...
	if caster:GetTeam() ~= target:GetTeam() then
		local damage_table = {
			...
			victim = target,
			attacker = caster,
			damage = keys.ability:GetAbilityDamage()+(caster:GetStrength()*1),
		...
		target:Heal(keys.heal_amount+(caster:GetStrength()*1)+target:GetMaxHealth()*keys.heal_percent/100, caster)
