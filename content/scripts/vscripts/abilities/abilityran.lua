...
function Ran03_OnSpellStart(keys)
	...
	local damage_table = {
		...
		attacker = caster, 
		ability = ability,
		damage = ability:GetAbilityDamage() + (caster:GetIntellect()*0.3),
	...
	local jump_count = 1
	local telentjumpCount = FindTalentValue(caster,"special_bonus_unique_puck")
	local jumpAmount = 1
	if telentjumpCount > 0 then
		jumpAmount = keys.JumpCount * telentjumpCount
	else
		jumpAmount = keys.JumpCount
	end

	if jumpAmount > 1 then
		caster:SetContextThink("ran03_lazer_jumping", function()
			if GameRules:IsGamePaused() then return 0.03 end
			local enemies = FindUnitsInRadius(
				caster:GetTeamNumber(),
				target_unit:GetAbsOrigin(),
				nil,
				keys.JumpRadius,
...
function OnRan04SpellStart(keys)
local caster = EntIndexToHScript(keys.caster_entindex)
	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_puck_2"))
end
