		...
		function(i)
	    	...
	    	for _,v in pairs(targets) do
				local damage_table = {
					...
					victim = v,
					attacker = caster,
					damage = keys.ability:GetAbilityDamage()+(caster:GetIntellect()*1),
...
function OnSanae02SpellStart(keys)
	...
	local StunDuration = keys.StunDuration+FindTalentValue(caster,"special_bonus_unique_lion_2")+FindTalentValue(caster,"special_bonus_unique_lion_3")
	for _,v in pairs(targets) do
		local damage_table = {
			ability = keys.ability,
			victim = v,
			attacker = caster,
			damage = keys.ability:GetAbilityDamage()+(caster:GetIntellect()*2),
			damage_type = keys.ability:GetAbilityDamageType(), 
			damage_flags = keys.ability:GetAbilityTargetFlags()
		}
		UnitDamageTarget(damage_table)
...
function OnSanae03SpellStart(keys)
	...
	target:Heal(keys.HealAmount + caster:GetMaxMana() * 0.1,caster)
	SendOverheadEventMessage(nil,OVERHEAD_ALERT_HEAL,target,keys.HealAmount + caster:GetMaxMana() * 0.1,nil)
