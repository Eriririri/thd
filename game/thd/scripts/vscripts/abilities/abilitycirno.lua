...
function OnCirno01SpellStart(keys)
	...
	if FindTalentValue(Caster,"special_bonus_unique_cirno_1")~=0 then
		keys.ability:ApplyDataDrivenModifier( Caster, v, "modifier_cirno_int_talent", {} )
	end		
...
function Cirno04animation(event)
...
function Cirno04animationend(event)
...
function OncirnoEXDealDamage(keys)
	local caster = keys.caster
	local target = keys.target
	local cirnoexdealdamage = (caster:GetStrength() - target:GetIntellect())*1.5
	local ability = keys.ability
	ability:ApplyDataDrivenModifier(caster, target, "modifier_cirnoEX_reduce_int", {})		
	if caster:GetStrength() > target:GetIntellect() then
			local damage_table = {
				ability = keys.ability,
			    victim = target,
			    attacker = caster,
			    damage = cirnoexdealdamage,
...
function cirnoEX(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local Caster=keys.caster
	local level = keys.ability:GetLevel()
    local ability = Caster:FindAbilityByName("ability_thdots_cirnoEX")
