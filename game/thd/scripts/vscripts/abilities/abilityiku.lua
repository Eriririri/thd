...
--雷云棘鱼
function OnIku02SpellStartcd(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local target = keys.target

	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_visage_6"))
end
function OnIku02SpellStart( keys )
	...
	--获取闪电伤害
	local light_damage = keys.ability:GetLevelSpecialValueFor("light_damage", i) + Iku01_light + FindTalentValue(caster,"special_bonus_unique_razor")
	...
	for i,unit in pairs(group) do
		local damageTable = {victim=unit,
							...
							damage=light_damage+(caster:GetAgility()*1)}
...
function OnIku03AttackLight( keys,caster,unit,lastUnit,caster_abs,time )
	...
	light_damage = (keys.ability:GetLevelSpecialValueFor("light_damage", i) + Iku01_light + FindTalentValue(caster,"special_bonus_unique_razor"))
	end
...
function OnIku03Attack( keys )
	...
	local basechance = keys.basechance
	
	
	if RollPercentage(basechance) and caster:HasModifier("modifier_ability_thdots_iku04_attack") == false  then	
	...
	--获取传递数量
	local num = keys.ability:GetLevelSpecialValueFor("num", i) + 1 + FindTalentValue(caster,"special_bonus_unique_Iku_1")
...
function OnIku04SpellStart( keys )
	...
	caster:EmitSound("Voice_Thdots_Iku.AbilityIku04_"..math.random(1,3))
	...
	for k,unit in pairs(group) do
		...
		if angle.y<caster_angle_max and angle.y>caster_angle_min and length<=radius then
			...
			local damageTable = {victim=unit,
								attacker=caster,
								damage_type=keys.ability:GetAbilityDamageType(),
								damage=light_damage+(caster:GetIntellect()*2.5)}
			ApplyDamage(damageTable)
		end
	end
end
...
function OnIku04Attack( keys )
	...
	if ability:GetLevel() >= 1 then
		local table = {caster=caster,
						target=target,
						ability=ability}	
		OnIku04Attack2(table)
	end
end

function OnIku04Attack2( keys )
	local caster = keys.caster
	local target = keys.target
	local lastUnit = caster
	local lastTarget = target
