function OnKoishi01Think(keys)
	...
	local countTelent = FindTalentValue(caster,"special_bonus_unique_drow_ranger_2")
...
function OnKoishi04PhaseStart(keys)
 	local caster = EntIndexToHScript(keys.caster_entindex)
 	if FindTalentValue(caster,"special_bonus_unique_drow_ranger_3") ~= 0 then
 		keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_koishi04_telent_bonus", {})
 	end
	if caster:HasItemInInventory("item_yuemianzhinu") then
 		keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_demon_bow_range", {})
...
function OnKoishi04ActionDestroy(keys)
	...
	keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_koishi04_no_heal", nil)
...
function OnKoishi04Think(keys)
	...
	if targets[1]~=nil and targets[1]:IsInvisible()==false then
...
function OnKoishi04HeroKill(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)

 	keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_koishi04_hero_kill", {})	
end

function koishicrit (keys)

	local caster = keys.caster
	local ability = keys.ability
	local skilllvl = ability:GetLevel()
	
	if skilllvl == 1 then
 		keys.ability:ApplyDataDrivenModifier(caster, caster, "koishi03_check_critical", {})
...
LinkLuaModifier("koishi05boostcap", "scripts/vscripts/modifiers/koishi/koishi05boostcap.lua", LUA_MODIFIER_MOTION_NONE)


function koishi05boost (keys)

	local ability = keys.ability

	local caster = keys.caster
	
	caster:AddNewModifier(caster, ability, "koishi05boostcap",  { Duration = keys.duration} )
