...
function MargatroidEx_IntervalAddDoll(keys)
	...
	local MaxStoreNum=keys.DollBaseNum+math.floor((Caster:GetLevel()-1)/keys.LvlToUpgrade) + FindTalentValue(castertalent,"special_bonus_unique_Margatroid_5")+FindTalentValue(castertalent,"special_bonus_unique_Margatroid_2")

	if Margatroid_GetTotalDollNum(Caster)<MaxStoreNum and not Caster:HasModifier(MARGATROID_ABILITYEX_MODIFIER_TIME_TO_ADD_DOLL_NAME) then
		AbilityEx:ApplyDataDrivenModifier(Caster,Caster,MARGATROID_ABILITYEX_MODIFIER_TIME_TO_ADD_DOLL_NAME,{Duration=5-FindTalentValue(castertalent,"special_bonus_unique_Margatroid_8")})
	end
...
function MargatroidEx_OnSpellStart(keys)
	...
	local dolls=Margatroid_FindDolls(Caster,keys.Radius)
...
function Margatroid01_OnSpellStart(keys)
	...
	THDReduceCooldown(keys.ability,-FindTalentValue(castertalent,"special_bonus_unique_Margatroid_4"))
...
function Margatroid01_IfKnockback(keys)
	...
	local margatroid01dealdamage = Ability:GetAbilityDamage() + FindTalentValue(castertalent,"special_bonus_unique_Margatroid_6") + (keys.caster:GetIntellect()*1)
	...
	if not Target:HasModifier(keys.ModifierKnockbackName) then
		...
		UnitDamageTarget{
			...
			victim = Target,
			attacker = Caster,
			damage = margatroid01dealdamage ,
			damage_type = Ability:GetAbilityDamageType(),
		}
			SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_SPELL_DAMAGE, Target, margatroid01dealdamage, nil)			
...
function Margatroid02_OnSpellStart(keys)
	...
	local margatroid02dealdamge = Ability:GetAbilityDamage() + FindTalentValue(castertalent,"special_bonus_unique_Margatroid_7") + (keys.caster:GetIntellect()*intscale)
						...
						for __,enemy in pairs(enemies) do
							...
							if math.abs(after_rotate_pos.y-doll_pos.y)<keys.LaserRadius then
								UnitDamageTarget{
									...
									victim = enemy,
									attacker = Caster,
									--damage = Ability:GetAbilityDamage() + FindTalentValue(castertalent,"special_bonus_unique_antimage_2") + (keys.caster:GetIntellect()*0.6),
									damage = margatroid02dealdamge,									
									damage_type = Ability:GetAbilityDamageType()
								}
								SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_SPELL_DAMAGE, enemy, margatroid02dealdamge, nil)								
							end
						end
					end,0.1)
			end
...
function Margatroid03_OnSpellStart(keys)
	...
	if Caster.ability_alice_03_doll~= nil and Caster.ability_alice_03_doll:IsNull() == false then
	--and GameRules:IsCheatMode()==false then
	Caster.ability_alice_03_doll:ForceKill(false)
	end
	local doll=CreateUnitByName(
	...
	Caster.ability_alice_03_doll = doll
	...
	doll:SetMaxHealth(keys.DollHP+keys.caster:GetMaxHealth())
	doll:SetPhysicalArmorBaseValue(keys.Armor+keys.caster:GetPhysicalArmorValue())
	doll:SetBaseMoveSpeed(keys.MoveSpeed+FindTalentValue(castertalent,"special_bonus_unique_Margatroid_3"))
	doll:CreatureLevelUp(0)
	doll:SetBaseDamageMax(keys.DollDamage + 26 + (keys.caster:GetIntellect()*1))
	doll:SetBaseDamageMin(keys.DollDamage + 26 + (keys.caster:GetIntellect()*1))
	doll:SetMana(400)
	doll:SetControllableByPlayer(Caster:GetPlayerID(),false)
	doll:SetBaseMagicalResistanceValue(keys.Resistance)
	doll:SetMaximumGoldBounty(keys.Maxgold)
	doll:SetMinimumGoldBounty(keys.Mingold)
	
	if FindTalentValue(Caster,"special_bonus_unique_Margatroid_1")~=0 then
	Ability:ApplyDataDrivenModifier(Caster,doll,"modifier_thdots_margatroid03_doll_talent",{})
	doll:SetMaximumGoldBounty(keys.Maxgold*2)
	doll:SetMinimumGoldBounty(keys.Mingold*2)	
	...
	if ability_lvl>=4 and keys.Level5Ability then
		local ability=doll:AddAbility(keys.Level5Ability)
		ability:SetLevel(1)
	end
...
function Margatroid03_DollOnIntervalThink(keys)
	...
	AddFOWViewer(Doll:GetTeamNumber(), Doll:GetAbsOrigin(), Doll:GetCurrentVisionRange(), 0.051, false)
...
function Margatroid32_OnSpellStart(keys)
	...
	local dealdamagealice32 = (Ability:GetAbilityDamage()+(keys.caster.ability_alice_03_owner:GetIntellect()*0.5))*(1+(FindTalentValue(Caster,"special_bonus_unique_Margatroid_1")))
	...
	UnitDamageTarget{
			...
			damage = dealdamagealice32,
			damage_type = Ability:GetAbilityDamageType()
		}   
		SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_SPELL_DAMAGE, Target, dealdamagealice32, nil)		
end
...
function Margatroid33_OnSpellStart(keys)
	...
	local margatroid33damage = (Ability:GetAbilityDamage()+(keys.caster.ability_alice_03_owner:GetIntellect()*1))*(1+(FindTalentValue(Caster,"special_bonus_unique_Margatroid_1")))
	
	local damage_table={
			ability = Ability,
			victim = nil,
			attacker = keys.caster.ability_alice_03_owner,
			damage = margatroid33damage,
			damage_type = Ability:GetAbilityDamageType()
		}  
	...
	for _,enemy in pairs(enemies) do
		...
		if after_rotate_pos.y-caster_pos.y<keys.LaserRadius then
			...
			SendOverheadEventMessage(nil, OVERHEAD_ALERT_BONUS_SPELL_DAMAGE, enemy, margatroid33damage, nil)			
		end
	end
end
...
function Margatroid04_OnSpellStart(keys)
	...
	THDReduceCooldown(keys.ability,-FindTalentValue(castertalent,"special_bonus_unique_alchemist_4"))
end
...
function Margatroid04_DamageTargets(keys)
	...
	local margatroid04damgex = keys.explosion_damage + FindTalentValue(Caster,"special_bonus_unique_visage_2") + (Caster:GetIntellect()*keys.intscale)

	for k,v in pairs(targets) do
	SendOverheadEventMessage(nil,  OVERHEAD_ALERT_BONUS_SPELL_DAMAGE , v, (margatroid04damgex) , nil)	
	local effectIndex = ParticleManager:CreateParticle("particles/newthd/alice/04/alice04explode.vpcf", PATTACH_CUSTOMORIGIN, nil)
   	ParticleManager:SetParticleControl(effectIndex, 0, Vector(v:GetOrigin().x,v:GetOrigin().y,v:GetOrigin().z + 90))	
	
	local effectIndex2 = ParticleManager:CreateParticle("particles/units/heroes/hero_techies/techies_blast_off.vpcf", PATTACH_CUSTOMORIGIN, nil)	
   	ParticleManager:SetParticleControl(effectIndex2, 0, v:GetOrigin())	
	local effectIndex3 = ParticleManager:CreateParticle("particles/units/heroes/hero_ogre_magi/ogre_magi_ignite_debuff_explosion.vpcf", PATTACH_CUSTOMORIGIN, nil)	
   	ParticleManager:SetParticleControl(effectIndex3, 0, v:GetOrigin())		
		UnitDamageTarget{
			ability = Ability,
			victim = v,
			attacker = Caster,
			damage = margatroid04damgex,
			damage_type = Ability:GetAbilityDamageType()
		}
		if keys.attacker:IsNull()~= true then 
		keys.attacker:Kill(Ability,keys.attacker)
		end
	end

end
function OnMargatroid03DollBack(keys)
	local caster = keys.caster
	caster:SetOrigin(caster.ability_alice_03_owner:GetOrigin())
...
function Margatroid34_DamageTargets(keys)
	local Ability=keys.ability
	local Caster=keys.caster
	local targets = keys.target_entities

	local margatroid34damage = (keys.explosion_damage +(keys.caster.ability_alice_03_owner:GetIntellect()*keys.intscale))*(1+(FindTalentValue(Caster,"special_bonus_unique_Margatroid_1")))
	
	for k,v in pairs(targets) do
		SendOverheadEventMessage(nil, OVERHEAD_ALERT_DAMAGE, v,margatroid34damage, nil)		
		UnitDamageTarget{
			ability = Ability,
			victim = v,
			attacker = Caster,
			damage = margatroid34damage,
			damage_type = Ability:GetAbilityDamageType()
		}	
	end
end

function OnMargatroid03DollAttack(keys)
	...
	if target:GetUnitName() == "npc_dota_sentry_wards" then
	...
	local dolldealdamage = attacker:GetBaseDamageMax()*0.5
		UnitDamageTarget{
			ability = ability,
			victim = target,
			attacker = attacker,
			damage = dolldealdamage,
...
function Margatroid04_Building (keys)
