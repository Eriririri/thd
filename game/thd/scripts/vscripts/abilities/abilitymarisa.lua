...
function OnMarisa03DealDamage(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local dealdamage = keys.DealDamage + caster.hero:GetIntellect()*keys.int_scale
...
function OnMarisa03SpellDisactivated(keys)

	keys.ability:SetActivated(false)

end

function OnMarisa03SpellActivated(keys)
...
function OnMarisa04SpellStart(keys)
	...
	THDReduceCooldown(keys.ability,FindTalentValue(caster,"special_bonus_unique_crystal_maiden_2"))
	
	EmitGlobalSound("Voice_Thdots_Marisa.AbilityMarisa04")	
	...
	for _,v in pairs(targets) do
		...
		if(v:GetContext("ability_marisa01_damage")==TRUE)then
			local damage_table = {
			    ...
			    victim = v,
			    attacker = caster,
			    damage = keys.ability:GetAbilityDamage()+(caster:GetIntellect()*0.9),
			    damage_type = keys.ability:GetAbilityDamageType(), 
	    	    damage_flags = 0
		    }
		    UnitDamageTarget(damage_table)
...
function AbilityMarisa:OnMarisa02Damage(keys)
	...
	for _,v in pairs(targets) do
		...
		if(IsPointInCircularSector(vVec.x,vVec.y,math.cos(vecRad),math.sin(vecRad),keys.DamageRadius,math.pi/3,vecCaster.x,vecCaster.y))then
			local deal_damage = (keys.ability:GetAbilityDamage()+(caster:GetIntellect()*1.6)+FindTalentValue(caster,"special_bonus_unique_crystal_maiden_3"))/5
			...
			if v:IsHero() ==false then
				if v:IsControllableByAnyPlayer() ~= true then
					local damage_table = {
					ability = keys.ability,
					victim = v,
					attacker = caster,
					damage = deal_damage*1.2,
					damage_type = keys.ability:GetAbilityDamageType(), 
					damage_flags = 0
					}
					UnitDamageTarget(damage_table)
...
function AbilityMarisa:OnMarisa03Start(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local duration = keys.AbilityDuration + FindTalentValue(caster,"special_bonus_unique_crystal_maiden_1")
	self.Marisa03Stars = {}
	-- ´´½¨ÐÇÐÇ
	for i = 0,(3+FindTalentValue(caster,"special_bonus_unique_jakiro")) do
...
function AbilityMarisa:OnMarisa04Think(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local marisaAbility1 = caster:FindAbilityByName("ability_thdots_marisa01")
	local marisaCooldown1 = marisaAbility1:GetCooldownTimeRemaining()
	if  marisaCooldown1 >0 
	 then
	 marisaAbility1:EndCooldown()
	end
	local vecCaster = caster:GetOrigin()
	...
	for _,v in pairs(DamageTargets) do
		...
		if(IsRadInRect(vecV,vecCaster,keys.DamageWidth,keys.DamageLenth,sparkRad))then
			local deal_damage = keys.ability:GetAbilityDamage()/20+(caster:GetIntellect()*2.45)/20
...
function OnMarisa01voice(keys)

local caster = keys.caster

	if caster:GetHealth() < caster:GetMaxHealth()*0.25 then

	caster:EmitSound("Marisa01_"..math.random(1,2))
	end
