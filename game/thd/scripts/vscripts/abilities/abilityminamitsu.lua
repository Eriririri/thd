			...
			function ()
				...
				if distance < keys.range then
				    ...
				    for k,v in pairs(targets) do
				    	if v == caster or v:GetClassname()=="npc_dota_roshan" or v:GetUnitName()=="ability_minamitsu_04_ship" then
...
function OnMinamitsu03AttackLanded(keys)
    ...
    for k,v in pairs(targets) do
    	local damage_table = {
			...
			victim = v,
			attacker = caster,
			damage = keys.AttackBonus + (caster:GetLevel()*3),
			damage_type = keys.ability:GetAbilityDamageType(), 
		    damage_flags = keys.ability:GetAbilityTargetFlags()
		}
		UnitDamageTarget(damage_table) 
...
function OnMinamitsu04ShipDeath(keys)
	...
	if hero ~= nil and hero:IsNull() == false then
		...
		if ability ~= nil then
			...
			local ability4 = hero:FindAbilityByName("ability_thdots_minamitsu04")
			local ability4_level = ability4:GetLevel() - 1			
			local shiprebuildtime = ability4:GetLevelSpecialValueFor("reborn_time",ability4_level)
			ability:StartCooldown(shiprebuildtime + FindTalentValue(hero,"special_bonus_unique_murasa_2"))
		end
	end
end
			...
			if distance_end >= 0 then
			    ...
			    for k,v in pairs(targets) do
			    	local damage_table = {
						...
						victim = v,
						attacker = hero,
						damage = keys.ability:GetAbilityDamage()+FindTalentValue(hero,"special_bonus_unique_murasa_3"),
						damage_type = keys.ability:GetAbilityDamageType(), 
					    damage_flags = keys.ability:GetAbilityTargetFlags()
					}
					UnitDamageTarget(damage_table) 
...
function OnMinamitsu05AttackLanded(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local target = keys.target
	if not target:IsTower() then
		if target:HasModifier(keys.modifier_name) == false then
			local damage_table = {
				ability = keys.ability,
				victim = target,
				attacker = caster,
				damage = keys.DamageDealt,
				damage_type = keys.ability:GetAbilityDamageType(), 
				damage_flags = keys.ability:GetAbilityTargetFlags()}
			UnitDamageTarget(damage_table)
...
function MinamitsuEX1(keys)
	...
	local Caster=keys.caster
	--local level = keys.ability:GetLevel()
    --local ability = Caster:FindAbilityByName("ability_thdots_minamitsu_talent")
		--ability:SetLevel(1)
		
	if FindTalentValue(caster,"special_bonus_unique_murasa_1")~=0 then

	caster:AddAbility("ability_thdots_minamitsu_talent"):SetLevel(1)
	
	caster:RemoveAbility("ability_thdots_minamitsu05talent")	
	caster:RemoveModifierByName("modifier_murasa_ex_upgrade")
...
function murasaboatvision(keys)
	local caster = keys.caster
...
function Murasa04talent(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	THDReduceCooldown(keys.ability,FindTalentValue(caster,"special_bonus_unique_murasa_2"))	
end