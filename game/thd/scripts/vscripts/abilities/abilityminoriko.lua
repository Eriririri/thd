function OnMinoriko01SpellStart(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local target = keys.target

	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_phantom_assassin_2"))
end
function OnMinoriko01ProjectileHitUnit(keys)
	...
	if caster:GetTeam() ~= target:GetTeam() then
		...
		local damage_table = {
			...
			victim = target,
			attacker = caster,
			damage = keys.ability:GetAbilityDamage()+(caster:GetStrength()*1)+FindTalentValue(caster,"special_bonus_unique_minoriko_1"),
		...
		for k,v in pairs(targets) do
			local damage_table = {
				...
				victim = v,
				attacker = caster,
				damage = keys.aoe_damage+(caster:GetStrength()*1)+FindTalentValue(caster,"special_bonus_unique_minoriko_1"),
				damage_type = keys.ability:GetAbilityDamageType(), 
			    damage_flags = keys.ability:GetAbilityTargetFlags()
			}
			UnitDamageTarget(damage_table) 
	...
	local minorikohealamount = (keys.heal_amount+(caster:GetStrength()*1)+FindTalentValue(caster,"special_bonus_unique_minoriko_1")+target:GetMaxHealth()*keys.heal_percent/100)
		target:Purge(false,true,false,true,false)
		
		THDHealTarget(caster,target,minorikohealamount)		
		
	--	target:Heal(minorikohealamount, caster)
	--	SendOverheadEventMessage(nil,OVERHEAD_ALERT_HEAL,target,minorikohealamount,nil)
...
function OnMinoriko02SpellStart(keys)
	...
	keys.ability:ApplyDataDrivenModifier( minoriko02, minoriko02, "aura_thdots_minoriko02_buff", {} )
	...
	keys.target:SetContextThink(DoUniqueString("Minoriko02RegenHealth"), 
		function ()
			if count < 4 then
				...
				THDHealTarget(keys.caster,keys.target,keys.heal_amount/4)				
...
function OnMinoriko04SpellStart(keys)
	...
	local time = 0
	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_minoriko_2"))
		...
		function ()
			...
			if caster:GetTeam() == target:GetTeam() then
				local targets = FindUnitsInRadius(
						   ...
						   target:GetOrigin(),	
						   nil,					
						   keys.radius + FindTalentValue(caster,"special_bonus_unique_huskar"),		
						   DOTA_UNIT_TARGET_TEAM_ENEMY,
						   keys.ability:GetAbilityTargetType(),
						   0,
						   FIND_CLOSEST,
