function OnMomiji01Spawn(keys)
	...
	count = 2 + FindTalentValue(caster,"special_bonus_unique_bounty_hunter")
	...
	for i=1,count do
		...
		unit:AddAbility("ability_system_criticalstrike_wolves")
		
		local ability = unit:FindAbilityByName("ability_system_criticalstrike_wolves")
		if ability ~= nil then
			ability:SetLevel(1)
		end
		
	local momiji01abilityy = keys.ability

	local leveltosetgold = 	momiji01abilityy:GetLevel()
		
	unit:SetMaximumGoldBounty(leveltosetgold*25)
	unit:SetMinimumGoldBounty(leveltosetgold*25)	
	unit:SetBaseDamageMax(keys.basedamage + caster:GetBaseDamageMax()*0.5)
	unit:SetBaseDamageMin(keys.basedamage + caster:GetBaseDamageMin()*0.5)	
...
function OnMomiji02Start(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	AddFOWViewer( caster:GetTeam(), keys.target_points[1], keys.Radius, 15, false)
	...
	THDReduceCooldown(keys.ability,FindTalentValue(caster,"special_bonus_unique_momiji_1"))
	local unit = CreateUnitByName(
...
function OnMomiji03Start(keys)
	...
	local dealdamage = keys.ability:GetAbilityDamage() + caster:GetIntellect()*keys.intscale
	local DamageTable = {
			victim = target, 
			attacker = caster, 
