...
function GetNueExtraDamage(caster)
	...
	local basedamage = ability:GetSpecialValueFor("damage") + FindTalentValue(caster,"special_bonus_unique_phantom_assassin")
...
function OnNue02SpellHit(keys)
	...
	local damage_table = {
			    ...
			    victim = keys.target,
			    attacker = caster,
			    damage = keys.ability:GetAbilityDamage() + (caster:GetIntellect()*1.5)+FindTalentValue(caster,"special_bonus_unique_nue_1") + GetNueExtraDamage(caster) * keys.damage_percent,
			    damage_type = keys.ability:GetAbilityDamageType(), 
	    	    damage_flags = keys.ability:GetAbilityTargetFlags()
	}
...
function OnNue04Start(keys)
		...
		function()
		    ...
		    for k,v in pairs(targets) do
		    	local damage_table = {
				    ...
				    victim = v,
				    attacker = caster,
				    damage = keys.ability:GetAbilityDamage()+ (caster:GetIntellect()*1.5) + GetNueExtraDamage(caster) * keys.damage_percent,
				    damage_type = keys.ability:GetAbilityDamageType(), 
		    	    damage_flags = keys.ability:GetAbilityTargetFlags()
				}
	...
	1.5)
	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_phantom_assassin_3"))
end