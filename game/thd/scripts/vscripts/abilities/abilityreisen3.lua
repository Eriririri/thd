...
function OnReisenOld02DEBUG(keys)
	local ability = keys.ability
	local caster = EntIndexToHScript(keys.caster_entindex)
		if caster:IsRealHero()==true and caster:GetHealth() == 0 then
		--local hero = caster.illusioncaster
...
function OnReisenOld04SpellStart(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	keys.ability:ApplyDataDrivenModifier(caster, caster, "ability_reisenold04_modifier", {Duration = keys.duration })
end