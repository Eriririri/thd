...
function OnRemilia01SpellStart(keys)
	...
	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_warlock_1"))
	local vecCaster = caster:GetOrigin()
	keys.ability.ability_remilia01_vecCaster = vecCaster
	
	caster:EmitSound("Remilia01_"..math.random(1,2))
...
function OnRemilia01SpellHit(keys)
	...
	local leveltoscale = keys.ability:GetLevel()*0.5	
	local dealdamage = keys.ability:GetAbilityDamage()+(caster:GetIntellect()*leveltoscale)
	local damage_table = {
...
--	caster:Heal(dealdamage*0.36,Caster)
	THDHealTarget(caster,caster,dealdamage*0.36)
...
function OnRemilia02SpellThink(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	caster.ability_remilia_02_time_count = caster.ability_remilia_02_time_count + 0.05
	local moveDistance = 750 * caster.ability_remilia_02_time_count
...
function OnRemilia02SpellRemove(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	local moveDistance = keys.Distance * caster.ability_remilia_02_time_count
...
function OnRemilia03Think(keys)
	...
	for _,unit in pairs(Targets) do
		...
		--local totalheal = Damage*0.36
		
		local magicresist = unit:GetMagicalArmorValue()
		local afterresist = (1-magicresist)
		--print(magicresist)
	--	Caster:Heal(Damage*0.36*afterresist,Caster)
		THDHealTarget(Caster,Caster,Damage*0.36*afterresist)		
	end
...
function OnRemilia04Think(keys)
	...
	local intscale = keys.caster:GetIntellect()*keys.intscalez*0.05	
	local Damage=(keys.DamagePerSec*0.5/10) + (intscale)
	...
	for _,unit in pairs(targets) do
		if unit and unit:IsAlive() then
			if (math.abs(unit:GetAbsOrigin().x-CasterPos.x)<=CrossWidth) or (math.abs(unit:GetAbsOrigin().y-CasterPos.y)<=CrossWidth) then
				...
				THDHealTarget2(Caster,Caster,DamageAmount*LifestealPercent)				
			end
		end
	end
...
function remiliaLifesteal( keys )
	local caster = EntIndexToHScript(keys.caster_entindex)
	local damage = keys.DealDamage
