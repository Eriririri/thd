...
function OnSanae01SpellStart(keys)
	...
	Timer.Loop 'OnSanae01SpellStart' (0, keys.Duration+FindTalentValue(caster,"special_bonus_unique_sanae_1"),
		function(i)
	    	...
	    	for _,v in pairs(targets) do
				...
				local damage_table = {
					...
					victim = v,
					attacker = caster,
					damage = keys.ability:GetAbilityDamage()+(caster:GetIntellect()*leveltoscale),
				...
				if FindTalentValue(caster,"special_bonus_unique_sanae_3") ~= 0 then
				keys.ability:ApplyDataDrivenModifier( caster, v, "modifier_sanae01_slow_talent", {Duration=1} )				
				end
		    end
		    return 1
		end
...
function OnSanae02cooldown(keys)
local caster = keys.caster

	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_sanae_2"))
...
function OnSanae02SpellStart(keys)
	...
	local StunDuration = keys.StunDuration+FindTalentValue(caster,"special_bonus_unique_lion_2")
	for _,v in pairs(targets) do
		local leveltoscale = keys.intscaleability	
		local damage_table = {
			ability = keys.ability,
...
function OnSanae03SpellStart(keys)
	...
	local sanaeheal = keys.HealAmount + (caster:GetLevel()*keys.BonusLevelHeal) + (caster:GetMaxMana() * keys.BonusHeal /100) 
--	SendOverheadEventMessage(nil,OVERHEAD_ALERT_HEAL,target,sanaeheal,nil)	
--	target:Heal(sanaeheal,caster)

	THDHealTarget(caster,target,sanaeheal)
	target:Purge(false,true,false,true,false)
end

function OnSanae04SpellStart(keys)
	local caster = EntIndexToHScript(keys.caster_entindex)
	THDReduceCooldown(keys.ability,FindTalentValue(caster,"special_bonus_unique_lion"))
