...
LinkLuaModifier("modifier_shikieiki_respawn", "scripts/vscripts/modifiers/shikieiki/modifier_shikieiki_respawn.lua", LUA_MODIFIER_MOTION_NONE)
...
function Shikieiki02_OnSpellStart(keys)
	...
	local Damage=(keys.DamageBase+keys.DamageAccusation*AccusationNum)*(FindTalentValue(castertalent,"special_bonus_unique_storm_spirit_3")+1)
	...
	THDReduceCooldown(keys.ability,-FindTalentValue(castertalent,"special_bonus_unique_storm_spirit_2"))
end
...
function Shikieiki03_OnAttackLanded(keys)
	...
	if Target:IsRealHero() then
		...
		if GetAccusationCount(Caster,Target)>=MaxAccusationNum then
			local Damage=keys.DamageOnMaxAccusation
			UnitDamageTarget{
				ability = keys.ability,
				victim = Target,
				attacker = Caster,
...
function Shikieiki04_ModifierDebuff_OnIntervalThink(keys)
	...
	Target:AddNewModifier(Target, Ability, "modifier_shikieiki_respawn",  { Duration = 0.2} )		
	UnitDamageTarget{
		ability = keys.ability,
		victim = Target,
		attacker = Caster,
...
function Shikieiki04_OnSpellStart(keys)
	local caster=keys.caster
	THDReduceCooldown(keys.ability,FindTalentValue(caster,"special_bonus_unique_storm_spirit"))
	local target = keys.target
	local ability = keys.ability
