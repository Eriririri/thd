...
function OnYoumu01SpellStart(keys)
	...
	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_morphling_3"))
...
function OnYoumu01Kill(keys)
	local caster = keys.caster
	local YoumuAbility1 = caster:FindAbilityByName("ability_thdots_Youmu01")
	local YoumuCooldown1 = YoumuAbility1:GetCooldownTimeRemaining()
	if  YoumuCooldown1 >0 
	 then
	 YoumuAbility1:EndCooldown()
	end
...
function OnYoumu04SpellStart(keys)
	...
	--if  YoumuCooldown1 >0 
	 ...
	 --YoumuAbility1:EndCooldown()
	--end
	--local YoumuAbility3 = caster:FindAbilityByName("ability_thdots_Youmu03")
	--local YoumuCooldown3 = YoumuAbility3:GetCooldownTimeRemaining()
	--if  YoumuCooldown3 >0 
	--then
	 --YoumuAbility3:EndCooldown()
	--end

	THDReduceCooldown(keys.ability,FindTalentValue(caster,"special_bonus_unique_juggernaut"))
...
function AbilityYoumu:OnYoumu01Move(keys)
    ...
    local ability1 = caster:FindAbilityByName("ability_thdots_youmu01")	
    local scale = keys.basescale
    local abilityscale = (ability1:GetLevel()*scale)	
	...
	if(count == 0.2)then
		...
		for _,v in pairs(targets) do
				local damage_table = {
					...
					victim = v,
					attacker = caster,
					--damage = keys.ability:GetAbilityDamage()+(caster:GetAgility()*1)+(keys.caster:GetAttackDamage()*0.5),
					damage = keys.ability:GetAbilityDamage()+(caster:GetAttackDamage()*abilityscale)+(caster:GetLevel()),
					damage_type = keys.ability:GetAbilityDamageType(), 
					damage_flags = keys.ability:GetAbilityTargetFlags()
				}
				
				ability1 = caster:FindAbilityByName("ability_thdots_youmu01")
				ability1:ApplyDataDrivenModifier( caster, v, "modifier_youmu01_armor_decrease", {} )				
				
				
				UnitDamageTarget(damage_table)
		--AbilityYoumu:OnYoumu02Start(keys)				
		--ability2 = caster:FindAbilityByName("ability_thdots_youmu02")		
		--ability2:ApplyDataDrivenModifier( caster, v, "modifier_youmu02_armor_decrease", {} )	
...
function OnYoumu05SpellStart(keys)
			local caster = keys.caster
	THDReduceCooldown(keys.ability,-FindTalentValue(caster,"special_bonus_unique_youmu_1"))			
	caster:Purge(false,true,false,true,false)
		local effectIndex = ParticleManager:CreateParticle("particles/thd2/heroes/youmu/youmu_01_blink_effect_e.vpcf", PATTACH_CUSTOMORIGIN, caster)
		ParticleManager:SetParticleControl(effectIndex, 0, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex, 1, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex, 2, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex, 5, caster:GetOrigin())
		...
		local effectIndex2 = ParticleManager:CreateParticle("particles/thd2/heroes/youmu/youmu_04_blossoms_effect.vpcf", PATTACH_CUSTOMORIGIN, caster)
		...
		ParticleManager:SetParticleControl(effectIndex2, 2, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex2, 5, caster:GetOrigin())

		ParticleManager:DestroyParticleSystem(effectIndex2,false)
		
		local effectIndex3 = ParticleManager:CreateParticle("particles/thd2/heroes/youmu/youmu_02_effect_sword_light_c.vpcf", PATTACH_CUSTOMORIGIN, caster)
		ParticleManager:SetParticleControl(effectIndex3, 0, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex3, 1, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex3, 2, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex3, 5, caster:GetOrigin())

		ParticleManager:DestroyParticleSystem(effectIndex3,false)
	
	
		local effectIndex4 = ParticleManager:CreateParticle("particles/thd2/heroes/youmu/youmu_01_blink_effect_c.vpcf", PATTACH_CUSTOMORIGIN, caster)
		ParticleManager:SetParticleControl(effectIndex4, 0, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex4, 1, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex4, 2, caster:GetOrigin())
		ParticleManager:SetParticleControl(effectIndex4, 5, caster:GetOrigin())

		ParticleManager:DestroyParticleSystem(effectIndex4,false)
		
	if FindTalentValue(caster,"special_bonus_unique_youmu_2") ~= 0 then
		local castertotalheal = caster:GetMaxHealth()*0.5	
		SendOverheadEventMessage(nil,OVERHEAD_ALERT_HEAL,caster,castertotalheal,nil)
		caster:EmitSound("lover_card")		
		caster:Heal(castertotalheal,caster)
	end			
