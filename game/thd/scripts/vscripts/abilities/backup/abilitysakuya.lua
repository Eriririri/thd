...
function OnSakuya01SpellReset(keys)
	...
	if(caster.sakuya04_cooldown_reset==TRUE)then
		...
		caster:SetMana(caster:GetMana() - usedCount * 0.00 * keys.ability:GetManaCost(keys.ability:GetLevel()))
...
function Sakuya04Talent(keys)
	...
	local target = keys.target	
	if FindTalentValue(caster,"special_bonus_unique_sakuya_1")~=0 then
		keys.ability:ApplyDataDrivenModifier( caster, target, "Illusion_world", {} )
		...
		local effectIndex2 = ParticleManager:CreateParticle(
			"particles/thd2/heroes/sakuya/ability_sakuya_04.vpcf", 
			PATTACH_CUSTOMORIGIN, 
			caster)		
		
		
		ParticleManager:SetParticleControlEnt(effectIndex2 , 0, target, 5, "follow_origin", Vector(0,0,0), true)
