...
function suika_toss:OnUpgrade()


	local caster = self:GetCaster()
	local ability = self
	local suika01level = ability:GetLevel()
    local ability1 = caster:FindAbilityByName("tiny_toss")
		ability1:SetLevel(suika01level)
	...
	if IsServer() then
		...
		for _, victim in pairs(victims) do
			...
			if victim:IsBuilding() then
				...
				ApplyDamage({victim = victim, attacker = caster, damage = damage+(caster:GetStrength()*0.85), damage_type = self.ability:GetAbilityDamageType(), ability = self.ability})
			else
				ApplyDamage({victim = victim, attacker = caster, damage = damage+(caster:GetStrength()*0.85), damage_type = self.ability:GetAbilityDamageType(), ability = self.ability})
			end
...
function modifier_suika_toss_scepter_bounce:OnRemoved()
	if IsServer() then
		...
		for _,enemy in pairs(enemies) do
			ApplyDamage({victim = enemy, attacker = self.caster, damage = damage + (caster:GetStrength()*2), damage_type = self:GetAbility():GetAbilityDamageType(), ability = self:GetAbility()})
		end
