...
function HeartstopperAura( keys )
	...
	local aura_damage = caster:GetIntellect()*0.015 +(ability:GetLevelSpecialValueFor("aura_damage", (ability:GetLevel() - 1)))
	...
	damage_table.damage = ((target_max_hp * aura_damage)+15) * aura_damage_interval
	damage_table.damage_flags = DOTA_DAMAGE_FLAG_HPLOSS -- Doesnt trigger abilities and items that get disabled by damage
	if target:IsAncient() ==false and target:GetUnitName() ~= "npc_tewi_remote_bomb_1" then
