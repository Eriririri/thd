...
function addstack( keys )
	...
	if caster:HasModifier("modifier_reimu_talent_2") then
...
function intdamage( keys )
	...
	print("damage =",damage)
	if caster:HasModifier("modifier_reimu_talent_1") then
	local damage_table = {}
...
function fatasysealdamage(keys)
	local caster = keys.caster
	
	local target = keys.target
	local ability = keys.ability
	--caster:EmitSound("Voice_Thdots_Tenshi.AbilityTenshi042EX_"..math.random(1,3))
	 ApplyDamage({victim = target, attacker = caster, damage = (caster:GetIntellect()*1) +  ability:GetAbilityDamage(), damage_type = keys.ability:GetAbilityDamageType()})
