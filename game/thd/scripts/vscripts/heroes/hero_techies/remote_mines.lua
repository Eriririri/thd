...
function RemoteMinesPlant( keys )
	...
	local remote_mine = CreateUnitByName("npc_tewi_remote_bomb_1", target_point, false, nil, nil, caster:GetTeamNumber())
	...
	Timers:CreateTimer(activation_time, function()
		...
		local aimeffect = keys.Radius
		caster.selfAuraBorderFx = ParticleManager:CreateParticleForTeam("particles/newthd/tewi/mines_aim/mines.vpcf", PATTACH_ABSORIGIN_FOLLOW, remote_mine, caster:GetTeamNumber())
...
function RemoteMinesSelfDetonate( keys )
	local caster = keys.caster
	--local playerx = caster:GetPlayerOwner()
	--local tewicaster = playerx:GetAssignedHero()	
	--local bombowner = caster:GetOwnerEntity()
...
function RemoteMinesFocusedDetonate( keys )
	...
	for _,unit in ipairs(units) do
		if unit:GetUnitName() == "npc_tewi_remote_bomb_1" then
			--unit:ForceKill(true) 
